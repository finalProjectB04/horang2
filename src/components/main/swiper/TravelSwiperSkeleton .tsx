      ...
      <div className={`flex ${isLgScreen ? "flex-row" : "flex-col lg:flex-row"} gap-4`}>
        {skeletonItems.map((_, index) => (
          <div
            ...
            } relative rounded-[6px] bg-gray-200 animate-pulse`}
          >
            <div className={`${isLgScreen ? "h-[150px]" : "lg:h-[224px] h-[110px]"} bg-gray-300 rounded-t-[6px]`}></div>
            <div
              className={`${
                isLgScreen
                  ? "w-full h-[130px] px-[10px] py-[20px] gap-[8px]"
                  : "lg:w-full lg:h-[122px] lg:px-[14px] lg:py-[28px] lg:gap-[10px] p-2 gap-1.5 h-[56px]"
              } bg-gray-100 rounded-b-[6px] flex flex-col`}
            >
              <div className={`${isLgScreen ? "h-6 w-3/4" : "lg:h-7 h-4 w-3/4"} bg-gray-300 rounded`}></div>
              <div className={`${isLgScreen ? "h-4 w-1/2" : "lg:h-5 h-3 w-1/2"} bg-gray-300 rounded mt-2`}></div>
            </div>
          </div>
        ))}
      </div>
