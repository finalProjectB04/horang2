...
export const GET = async (request: NextRequest, { params }: { params: { MBTIResult: string } }) => {
  ...
  if (!MBTIResult) {
    return NextResponse.json({ error: "MBTIResult parameter is required" }, { status: 400 });
  }
  ...
  switch (MBTIResult) {
    ...
    default:
      return NextResponse.json({ error: "Invalid MBTI Result" }, { status: 400 });
  }
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      NextResponse.json({ error: "Error fetching data", details: error.message });
    } else {
      console.error("Unexpected error:", error);
      NextResponse.json({ error: "Unexpected error", details: error });
    }
  }
};
